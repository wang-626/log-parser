...
async function run(args) {
  ...
  const now = new Date();
  data[0] = 0x55; data[1] = centerId; data[2] = TOKEN_RS485.CTR_GET_POWER_METER_DATA_110; data[3] = meterId;
  ...
  const result = await executeTransaction(data, [TOKEN_RS485.CTR_RSP_POWER_METER_DATA_110]);
  if (result.success) {
    broadcast({ new_log: [nowTime(), ...Array.from(result.response)] });
    const payload = { cmd: 'RoomPowerMeterData110', status: 'success', centerId, meterId, response: Array.from(result.response) };
    broadcast(payload);
    return JSON.stringify(payload);
  } else {
    const payload = { cmd: 'RoomPowerMeterData110', status: 'failed', centerId, meterId, reason: result.reason };
    broadcast(payload);
    throw new Error(`RoomPowerMeterData110 failed center:${centerId} meter:${meterId} reason:${result.reason}`);
  }
}
