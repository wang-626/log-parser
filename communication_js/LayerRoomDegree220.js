...
async function run(args) {
  ...
  const now = new Date();
  data[0] = 0x55; data[1] = centerId; data[2] = TOKEN_RS485.CTR_GET_POWER_METER_220; data[4] = 1;
  ...
  const result = await executeTransaction(data, [TOKEN_RS485.CTR_RSP_POWER_METER_220]);
  if (result.success) {
    broadcast({ new_log: [nowTime(), ...Array.from(result.response)] });
    const payload = { cmd: 'LayerRoomDegree220', status: 'success', centerId, response: Array.from(result.response) };
    broadcast(payload);
    return JSON.stringify(payload);
  } else {
    const payload = { cmd: 'LayerRoomDegree220', status: 'failed', centerId, reason: result.reason };
    broadcast(payload);
    throw new Error(`LayerRoomDegree220 failed center:${centerId} reason:${result.reason}`);
  }
}
