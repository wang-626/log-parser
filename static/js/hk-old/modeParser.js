...
class Mode18Parser extends ModeParser {
  ...
  */
  userInfo() {
    ...
    <thead><tr><th>學生ID</th><th>送電狀態</th><th>Byte</th><th>卡號</th><th>Byte</th><th>學號</th><th>Byte</th><th>餘額</th><th>Byte</th></tr><tbody>`
    let curr = 7
    for (let i = 0; i < 6; i++) {
      ...
      curr += 4
      const sid = this.Bidcard_to_str(this.bytes.slice(curr, curr + 4))
      const sidByte = `${curr} - ${curr + 3}`
      curr += 4
      const balance = this.Bbalance_to_str(this.bytes.slice(curr, curr + 4))
      const balanceByte = `${curr} - ${curr + 3}`
      curr += 4
      html += `<tr><td>${id}</td><td>${mode}</td><td>${modeByte}</td><td>${uid}</td><td>${uidByte}</td><td>${sid}</td><td>${sidByte}</td><td>${balance}</td><td>${balanceByte}</td></tr>`
    }
    html += `<tbody></table>`
    return html
  }
